...
%A function called by plotdisplayKera
    ...
    if rawAvailable %is the "raw" data available, in addition to the discretized data?
        handles.btn7 = uicontrol('Style', 'pushbutton', 'String', 'Threshold',...
            'Position', [660 5 95 20],...
            'UserData', 9, 'Callback', @buttonCallback);
    end
    
    if thresholded %has a threshold been applied to this data? if so, there should be a button to undo that
        ...
        handles.btn9 = uicontrol('Style', 'pushbutton', 'String', 'Reset All',...
            ...
            'UserData', 11, 'Callback', @buttonCallback);
    else
        handles.btn9 = uicontrol('Style', 'pushbutton', 'String', 'Reset All',...
            'Position', [765 5 100 20],...
            'UserData', 11, 'Callback', @buttonCallback);
    end
