...
%                 j0 = 2^(j-1);
%                 binM(:,j) = zeros([sum(timeM(:,2))+1,1],'uint8');
                count = 1;
...
%          matrix = testDataEbFRET(channels, stateList, 400, 600)'; %load test data matrix
         [file, path, ~] = uigetfile;
...
%     timeDataTemp = timeDataTemp';
%     nonZerosTemp = nonZerosTemp';
